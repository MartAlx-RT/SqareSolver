...
struct RefSolutions
{
    ...
    struct Roots RefRoots = {NAN, NAN, ZERO_SOL};
};
//--------------------UNITTEST--------------------------
...
void UnitTestWeak(struct Roots (*f)(struct Coefficients coeff), struct RefSolutions *sol, int NumOfTests);

int EnterTest(struct RefSolutions* sol, const char* FileName, const int size)
{
...
int main(void)
{
    ...
    struct RefSolutions *sol = (struct RefSolutions *)calloc(10, sizeof(struct RefSolutions));
...
static inline void SmartScanfDouble(double* x) //TODO сканф с проверкой //TODO assert(x), assert(x != y)
{
...
void Test1(struct Roots (*FuncUnderTest) (struct Coefficients coeff)) //D>0, two roots
{
    ...
    for (int i = 0; i < 100; i++)
    {
        ...
        if (!(Check(coeff, rts.Root1) && Check(coeff, rts.Root2)
...
void Test2(struct Roots (*FuncUnderTest) (struct Coefficients coeff)) //D=0
{
    ...
    for (int i = 0; i < 100; i++)
    {
        ...
        rts = FuncUnderTest(coeff);
        if(!(Check(coeff, rts.Root1) && Check(coeff, rts.Root2) && rts.NumOfRoots == ONE_SOL))
...
void UnitTest(struct Roots (*FuncUnderTest) (struct Coefficients coeff))
{
    Test1(FuncUnderTest);
    Test2(FuncUnderTest);
    Test3(FuncUnderTest);
...
void UnitTestWeak(struct Roots (*FuncUnderTest) (struct Coefficients coeff), struct RefSolutions* sol, int NumOfTests)
{
